            ...
            <div className="relative elegant-card rounded-xl sm:rounded-2xl overflow-hidden border border-[#EFC0BC]/80 premium-shadow hover:border-[#B18B81] transition-all duration-300" style={{ backgroundColor: "#FFE4E4" }}>
              ...
              <div className="relative p-4 sm:p-6 md:p-8 lg:p-10">
                <div className="flex items-center justify-center gap-2.5 sm:gap-3 md:gap-4 mb-3 sm:mb-4 md:mb-5">
                  <div className="bg-[#676B57] p-1.5 sm:p-2 md:p-2.5 rounded-full shadow-lg">
                    <Heart className="text-[#FFE4E4] h-3.5 w-3.5 sm:h-4 sm:w-4 md:h-5 md:w-5" fill="#FFE4E4" />
                  </div>
                  <div className="flex flex-col items-center">
                    <h3 className="text-base sm:text-lg md:text-xl lg:text-2xl font-[family-name:var(--font-crimson)] font-semibold text-[#676B57]">
                      {totalGuests} {totalGuests === 1 ? "Guest" : "Guests"}{" "}
                      Celebrating With Us
                    </h3>
                    <p className="text-[10px] sm:text-xs md:text-sm text-[#8C8E7C] font-[family-name:var(--font-crimson)] mt-0.5 sm:mt-1">
                      {guests.length}{" "}
                      {guests.length === 1 ? "RSVP entry" : "RSVP entries"}
                    </p>
                  </div>
                </div>
                <p className="text-xs sm:text-sm md:text-base text-[#676B57] font-[family-name:var(--font-crimson)] leading-relaxed">
              ...
              {isLoading ? (
                <div className="flex items-center justify-center py-16 sm:py-20 md:py-24">
                  <div className="flex flex-col items-center gap-3 sm:gap-4">
                    <Loader2 className="h-10 w-10 sm:h-12 sm:w-12 animate-spin text-[#676B57]" />
                    <span className="text-[#676B57] font-[family-name:var(--font-crimson)] text-sm sm:text-base md:text-lg">
                      Loading guests...
                    </span>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-center py-16 sm:py-20 md:py-24">
                  <div className="text-center">
                    <div className="bg-[#676B57] w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 rounded-full flex items-center justify-center mx-auto mb-3 sm:mb-4">
                      <Heart className="h-6 w-6 sm:h-7 sm:w-7 md:h-8 md:w-8 text-[#FFE4E4]" fill="#FFE4E4" />
                    </div>
                    <h3 className="text-lg sm:text-xl md:text-2xl font-[family-name:var(--font-crimson)] font-semibold text-[#676B57] mb-2">
                      No guests have RSVP'd yet
                    </h3>
                    <p className="text-xs sm:text-sm md:text-base text-[#8C8E7C] font-[family-name:var(--font-crimson)] max-w-md mx-auto leading-relaxed">
                ...
                <div className="mb-3 sm:mb-5 md:mb-7 lg:mb-9">
                  <div className="space-y-2.5 sm:space-y-3 md:space-y-4">
                    {guests.map((guest, index) => (
                      <div
                        key={index}
                        className="group relative bg-[#FFE4E4] rounded-lg sm:rounded-xl p-3 sm:p-4 md:p-5 border border-[#EFC0BC]/80 hover:border-[#B18B81] transition-all duration-300 hover:shadow-lg"
                      >
                      <div className="flex flex-col sm:flex-row gap-2.5 sm:gap-3 md:gap-4">
                        {/* Avatar */}
                        <div className="relative h-9 w-9 sm:h-11 sm:w-11 md:h-12 md:w-12 flex-shrink-0">
                          <div className="h-full w-full rounded-full bg-[#676B57] text-[#FFE4E4] flex items-center justify-center font-[family-name:var(--font-crimson)] font-semibold shadow-md ring-2 ring-white text-xs sm:text-sm md:text-base">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 sm:gap-3">
                            <div className="flex-1 pr-12 sm:pr-0">
                              <h4 className="font-[family-name:var(--font-crimson)] text-sm sm:text-base md:text-lg font-semibold text-[#676B57] mb-0.5 sm:mb-1 group-hover:text-[#676B57]/80 transition-colors duration-200">
                                {guest.Name}
                              </h4>
                              {guest.Email && guest.Email !== "Pending" && (
                                <div className="flex items-center text-[10px] sm:text-xs md:text-sm text-[#8C8E7C]">
                                  <Mail className="h-2.5 w-2.5 sm:h-3 sm:w-3 md:h-4 md:w-4 mr-1 sm:mr-1.5 text-[#676B57] flex-shrink-0" />
                            ...
                            <div className="absolute right-2.5 top-2.5 sm:static sm:right-auto sm:top-auto flex items-center gap-1 sm:gap-1.5">
                              <User className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-[#676B57] flex-shrink-0" />
                              <span className="inline-flex items-center justify-center px-2 sm:px-2.5 md:px-3 py-0.5 sm:py-1 bg-[#676B57]/10 text-[#676B57] rounded-full text-[10px] sm:text-xs md:text-sm font-[family-name:var(--font-crimson)] font-semibold border border-[#676B57]/30">
                                {guest.Guest
                          ...
                          {guest.Message && (
                            <div className="mt-2.5 sm:mt-3 md:mt-4 pt-2.5 sm:pt-3 md:pt-4 border-t border-[#800A06]/20">
                              <div className="flex items-start gap-2 sm:gap-2.5 md:gap-3">
                                <MessageSquare className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-[#676B57] flex-shrink-0 mt-0.5" />
                                <p className="text-[10px] sm:text-xs md:text-sm text-[#676B57]/80 font-[family-name:var(--font-crimson)] leading-relaxed italic flex-1">
                                  "{guest.Message}"
                                </p>
                              </div>
                            </div>
